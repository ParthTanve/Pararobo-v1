...
def refresh_intern_cache():
    try: get_all_interns.clear()
    except: pass
...
@st.cache_data(ttl=3600)
def get_month_stats(intern_name, target_month, target_year):
    ...
    if target_year == today.year and target_month == today.month:
        days_to_check = today.day
    elif target_year < today.year or (target_year == today.year and target_month < today.month):
        days_to_check = calendar.monthrange(target_year, target_month)[1]
    else:
        days_to_check = 0 
...
def mark_checkin_db(date_str, intern_name, time_str, photo_bytes, is_late=False, is_half_day_intent=False):
    ...
    if is_late:
        past_lates = db.collection("intern_attendance").where("intern_name", "==", intern_name).where("is_late", "==", True).get()
        late_count = len(past_lates)
        
        if late_count == 0:
            comment = "System Note: Late Check-In (1st Offense - Half Day Penalty)"
        else:
            comment = f"System Note: Late Check-In ({late_count + 1} Offenses - Absent Penalty)"
    ...
    db.collection("intern_attendance").add({
        ...
        "is_late": is_late,
        "late_count": (late_count + 1) if is_late else 0,
...
def show_intern_profile(intern):
    ...
    if user_role == 'Admin':
        with st.expander("✏️ Edit Profile (HR Only)"):
            ...
            r_opts = ["AI/ML Developer", "FullStack Developer", "Word Press Developer", "Frontend Developer", "Backend Developer", "Digital Marketing", "Sales & Marketing"]
...
def show_intern_page():
    ...
    if main_tab == "🧑‍🎓 Interns Information":
        ...
        if len(df_pending) == 0:
            ...
            for idx, row in df_pending.iterrows():
                with st.container(border=True):
                    ...
                    if is_half_day_intent:
                        approve_btn_text = "✅ Approve (Half Day)"
                    ...
                    with c2:
                        ...
                        req_color = "#ff9900"
                        if is_late or is_half_day_intent: req_color = "#ff4b4b"
                        ...
                        if is_half_day_intent:
                            st.markdown("<span style='color:#3498db; font-size: 13px;'>ℹ️ Note: Intern selected Half Day slot. Will be marked Half Day.</span>", unsafe_allow_html=True)
                        elif is_late:
                            st.markdown(f"<span style='color:#ff9900; font-size: 13px;'>⚠️ Warning: Late Offenses: {late_count}. Action Required.</span>", unsafe_allow_html=True)
                    ...
                    with c3:
                        ...
                        if is_checkin and is_late:
                            # 🟢 NEW: Manual control for HR on late check-ins
                            bc1, bc2 = st.columns(2)
                            with bc1:
                                if st.button("✅ Present", key=f"app_pres_{row['rowid']}", use_container_width=True):
                                    if ' '.join(conf_input.split()).lower() == ' '.join(row['intern_name'].split()).lower():
                                        update_attendance_status(row['rowid'], "Working")
                                        st.success("Marked Present!"); time.sleep(1); st.rerun()
                                    else: st.error("⚠️ Type name exactly.")
                            with bc2:
                                if st.button("❌ Absent", key=f"app_abs_{row['rowid']}", use_container_width=True):
                                    if ' '.join(conf_input.split()).lower() == ' '.join(row['intern_name'].split()).lower():
                                        update_attendance_status(row['rowid'], "Absent")
                                        st.error("Marked Absent!"); time.sleep(1); st.rerun()
                                    else: st.error("⚠️ Type name exactly.")
                        else:
                            # 🟢 NORMAL: For on-time checkins, checkouts, or half-day intents
                            bc1, bc2 = st.columns(2)
                            with bc1:
                                if st.button(approve_btn_text, key=f"app_{row['rowid']}", use_container_width=True):
                                    if ' '.join(conf_input.split()).lower() == ' '.join(row['intern_name'].split()).lower():
                                        update_attendance_status(row['rowid'], target_status)
                            ...
                            with bc2:
                                if st.button("❌ Reject", key=f"rej_{row['rowid']}", use_container_width=True):
                                    if ' '.join(conf_input.split()).lower() == ' '.join(row['intern_name'].split()).lower():
                                        ...
                                        st.error("Rejected!"); time.sleep(1); st.rerun()
                                    else: st.error("⚠️ Type name exactly.")
        ...
        if log_type == "📅 Attendance Log":
                ...
                if photo:
                    ...
                    if internship_type == "Part-Time Internship":
                        ...
                        if sel_intern in ["Kalyani Bhuyarkar", "Surbhi Harde"]:
                            c_in_s, c_in_e, c_out_s, c_out_e = datetime.time(10, 50), datetime.time(11, 00), datetime.time(18,25), datetime.time(19, 00)
                            msg_in, msg_out = "Check-In hasn't started yet (Starts at 10:50 AM).", "Check-Out only allowed 06:25 PM - 07:00 PM."
                        else:
                    ...
                    if att_action == "Check-In":
                        if current_time < c_in_s:
                            is_disabled, time_msg = True, msg_in
                        elif current_time > c_in_e:
                            is_late_checkin, time_msg = True, "Note: You are late. HR Approval required."
                    ...
                    if st.button(f" Confirm {att_action}", use_container_width=True, disabled=(is_disabled or is_att_disabled), key="btn_confirm_att"):
                        ...
                        if att_action == "Check-In":
                            ...
                            applied_status = mark_checkin_db(date_str, sel_intern, time_str, photo_bytes, is_late=is_late_checkin, is_half_day_intent=is_half_day)
                            if applied_status:
                                if is_late_checkin:
                                    st.warning("Late Check-In requested. Sent for HR Approval (Penalty will apply).")
                                else:
                                    st.success("Check-In requested. Sent for HR Approval!")
            ...
            all_absent_docs = db.collection("intern_attendance").where("intern_name", "==", sel_intern).where("status", "==", "Absent").get()
            total_absents = len(all_absent_docs)
            
            if total_absents == 8:
                st.error("🚨 ALERT: you have only 2 holiday as been left")
            elif total_absents == 9:
                st.error("🚨 ALERT: you have only 1 holiday as been left")
            elif total_absents >= 10:
                st.error("🚨 ALERT: you have exhausted all your 10 holidays!")
            
            present, absent, off, half_day, late_count = get_month_stats(sel_intern, sel_month, sel_year)
            ...
            is_task_time = (current_hour > 18) or (current_hour == 18 and current_minute >= 25) or (current_hour < 9)
            ...
            with st.container(border=True):
                ...
                if not current_task_data:
                    ...
                    if not is_task_time:
                        st.warning("⚠️ Evening Task Update is only active from 6:25 PM to 9:00 AM.")
                    ...
                    with st.spinner("Preparing and sending professional email to HR..."):
                        try:
                            ...
                            try:
                                ...
                                st.error("⚠️ Authentication Error: Gmail blocked the login. Please check if you are using a 16-digit 'App Password' instead of your normal password.")
                            except Exception as smtp_e:
                                st.error(f"⚠️ SMTP Connection Error: {smtp_e}")

                        except Exception as e:
                            st.error(f"⚠️ Failed to send email. General Error: {e}")