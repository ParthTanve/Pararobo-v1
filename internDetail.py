...
def mark_checkout_db(date_str, intern_name, time_str):
    ...
    docs[0].reference.update({"check_out": time_str, "status": "Pending Check-Out"})
    return "success"
...
def get_pending_attendances():
    ...
    for doc in docs:
        ...
        data_list.append({
            ...
            "status": d.get("status"), "photo_data": photo_bytes
        })
...
def save_task_log_db(date_str, day_str, intern_name, task, result, outcome, extra):
...
def create_task_log_table(df):
    ...
    for col in ["Name", "Date", "Day", "Today's Task", "Result", "Outcome", "Extra Curriculum"]: 
        html_table += f"<th style='padding: 12px;'>{col}</th>"
    html_table += "</tr>"
    for _, row in df.iterrows(): 
        task_val = row["Today's Task"]
        res_val = row.get("Result", "-") 
        html_table += f"<tr><td style='padding: 12px;'><strong>{row['Name']}</strong></td><td style='padding: 12px;'>{row['Date']}</td><td style='padding: 12px;'>{row['Day']}</td><td style='padding: 12px;'>{task_val}</td><td style='padding: 12px;'>{res_val}</td><td style='padding: 12px;'>{row['Outcome']}</td><td style='padding: 12px;'>{row['Extra Curriculum']}</td></tr>"
        ...
        if log_type == "📅 Attendance Log":
            ...
            with st.container(border=True):
                ...
                if has_submitted_today:
                    st.info(f"✅ {sel_intern_task} has already submitted the task log for today ({today_date}). You can view it below.")
                else:
                    ...
                    if st.button(" Submit Task Log", use_container_width=True, type="primary", key="btn_submit_task"):
                        if not task_input.strip() or not result_input.strip() or not outcome_input.strip(): 
                            ...
                            save_task_log_db(today_date, today_day, sel_intern_task, task_input, result_input, outcome_input, extra_input if extra_input.strip() else "-")
            ...
            with view_date_col:
                selected_view_date = st.date_input("Select Date (Calendar Picker)", datetime.date.today(), key="view_task_date")
