...
from email.message import EmailMessage # 🟢 Added for SMTP
import streamlit.components.v1 as components
from utils import load_global_css
import pytz

# 🟢 NAYA LOGIC: Firebase Database (db) aur config (Added SMTP variables)
from config import is_valid_email, db, SMTP_EMAIL, SMTP_PASSWORD
...
# 🟢 NAYA FIX: Is function ko cache kiya taaki har click par saare projects sync na hon
...
def add_new_intern(i_id, name, email, contact, role, project, comp_proj, mentor, duration, status, college, branch, sem, skills, photo, process, i_type):
    ...
    get_all_interns.clear() # 🟢 NAYA FIX: Cache clear kiya
    return "success"
...
def get_month_stats(intern_name):
    ...
    # 🟢 FIX: Database se intern ki Joining (Start) Date nikalna
    intern_docs = db.collection("interns").where("name", "==", intern_name).get()
    start_date = None
    if intern_docs:
        duration_str = intern_docs[0].to_dict().get("duration", "")
        try:
            s_str = duration_str.split(" to ")[0]
            start_date = datetime.datetime.strptime(s_str, '%d-%b-%Y').date()
        except:
            pass
    ...
    for day in range(1, today.day + 1):
        ...
        # 🟢 FIX: Joining date se pehle ka din skip kar dega, stats me nahi judega
    ...
    return present, absent, off, half_day
    ...
    for week in cal:
        ...
        for i, day in enumerate(week):
            if day == 0: html += "<td class='empty'></td>"
                ...
                elif start_date and d < start_date: display_status = "" # 🟢 FIX: Joining date se pehle bilkul khali (blank) dikhega
...
# 🟢 PHASE 1: Subha planned task save karne ka function
def save_planned_task_db(date_str, day_str, intern_name, task):
    db.collection("intern_daily_tasks").add({
        "date": date_str, "day": day_str, "intern_name": intern_name, "task": task,
        "result": "-", "outcome": "-", "extra_curriculum": "-", "submit_time": "-",
        "status": "Pending" 
    })
    get_all_task_logs_db.clear() # 🟢 NAYA FIX: Cache clear kiya

# 🟢 PHASE 2: Shaam ko yes/no ke sath update karne ka function
def update_actual_task_db(date_str, intern_name, result, outcome, extra, status, submit_time):
    docs = db.collection("intern_daily_tasks").where("date", "==", date_str).where("intern_name", "==", intern_name).get()
    if docs:
        docs[0].reference.update({
            "result": result, "outcome": outcome, "extra_curriculum": extra,
            "submit_time": submit_time, "status": status
        })
        get_all_task_logs_db.clear() # 🟢 NAYA FIX: Cache clear kiya

# 🟢 NAYA FIX: Logs ko cache kiya taaki list bar bar load na ho
@st.cache_data(ttl=3600)
def get_all_task_logs_db(intern_name=None, role="Admin"):
    ...
    for doc in docs:
        ...
        data_list.append({
            ...
            "Status": d.get("status", "Completed"), # 🟢 Added Status field for table
...
# 🟢 Task fetch karne ka logic
def get_daily_task_db(date_str, intern_name):
    docs = db.collection("intern_daily_tasks").where("date", "==", date_str).where("intern_name", "==", intern_name).get()
...
# 🟢 UPDATE: Task logs display function to add Status Column visually
...
def create_attendance_table(df):
    ...
    for _, row in df.iterrows():
        ...
        # 🟢 FIX: Table mein Half Day ki value ko bhi blue de diya
...
def show_intern_page():
    ...
    # 🟢 100% BULLETPROOF CSS FIX: Force text inside tertiary buttons to strictly left-align (Global impact on page load)
    st.markdown("""
    <style>
    ...
    tabs = ["🧑‍🎓 Interns Information", "📝 Intern Log", "📧 Send Mail"]
    if user_role == "Admin":
    ...
    if main_tab == "🧑‍🎓 Interns Information":
        ...
        # --- SECTION 2: Manual Override (Present / Half Day / Absent) ---
        st.markdown("<h4 style='color: #39FF14;'>2️⃣ Manual Attendance Override</h4>", unsafe_allow_html=True)
        st.markdown("<p style='color: #a1a1aa;'>If an intern forgot to check-in/out or lied, mark them Present/Half Day/Absent directly with a mandatory reason.</p>", unsafe_allow_html=True)
        ...
        with st.container(border=True):
            ...
            with m_col2:
                man_comment = st.text_area("Reason / Comment (Mandatory) *", placeholder="E.g., Forgot to check-in, left early for emergency, absent without notice...", height=110, key="man_override_comment")
            
            # 🟢 NAYA FIX: 3 Buttons add kiye (Present, Half Day, Absent)
            ...
            with b_col1:
                if st.button("✅ Mark as Present", type="primary", use_container_width=True, key="btn_man_present"):
                    ...
                    # Smart Validation: Handles extra internal spaces
                        ...
                        st.rerun()
            with b_col2:
                if st.button("🌗 Mark as Half Day", use_container_width=True, key="btn_man_halfday"):
                    if not man_comment.strip():
                        st.error("⚠️ Please enter a reason (comment) before marking half day!")
                    # Smart Validation: Handles extra internal spaces
                    elif ' '.join(man_conf_input.split()).lower() != ' '.join(man_intern.split()).lower():
                        st.error("⚠️ Type intern name exactly to confirm.")
                    else:
                        man_date_str = man_date.strftime('%d-%b-%Y')
                        manual_override_attendance_db(man_date_str, man_intern, 'Half Day', man_comment.strip())
                        st.success(f"Successfully marked {man_intern} as Half Day for {man_date_str}!")
                        time.sleep(1)
                        st.rerun()
            with b_col3:
        ...
        if log_type == "📅 Attendance Log":
            with st.container(border=True):
                ...
                if photo:
                    ...
                    if internship_type == "Part-Time Internship":
                        ...
                        if slot == "10:00 AM to 14.00 PM":
                            checkin_start, checkin_end, checkout_start, checkout_end = datetime.time(9, 50), datetime.time(10, 10), datetime.time(14,00), datetime.time(14,30)
                            time_msg_in, time_msg_out = " Check-In is only allowed between 09:50 AM and 10:10 AM for Slot 1.", " Check-Out is only allowed between 02:00 PM and 2:30 PM for Slot 1."
                        else: 
                            checkin_start, checkin_end, checkout_start, checkout_end = datetime.time(13, 50), datetime.time(14, 10), datetime.time(18,20), datetime.time(19, 00)
                            time_msg_in, time_msg_out = " Check-In is only allowed between 01:50 PM and 02:10 PM for Slot 2.", " Check-Out is only allowed between 06:20 PM and 07:00 PM for Slot 2."
                    else: 
                        checkin_start, checkin_end, checkout_start, checkout_end = datetime.time(9, 50), datetime.time(10, 10), datetime.time(18,20), datetime.time(19, 00)
                        time_msg_in, time_msg_out = " Check-In is only allowed between 09:50 AM and 10:10 AM.", " Check-Out is only allowed between 06:20 PM and 07:00 PM."
                    ...
                    if st.button(f" Confirm {att_action}", use_container_width=True, disabled=(is_disabled or is_att_disabled), key="btn_confirm_att"):
                        ...
                        # 🟢 FIX: Compress image so it never exceeds Firebase 1MB limit
                        try:
            ...
            # 🟢 FIX: Ab yahan 4 columns aayenge (Present, Half Day, Absent, Off)
            st.markdown("<h4 style='color: #ffffff; margin-top: 25px;'>📊 Current Month Stats</h4>", unsafe_allow_html=True)
            present, absent, off, half_day = get_month_stats(sel_intern)
            ...
            stat_col2.markdown(f"<div style='background:rgba(52, 152, 219, 0.1); border:1px solid #3498db; padding:15px; border-radius:8px; text-align:center;'><h3 style='color:#3498db; margin:0;'>{half_day}</h3><p style='color:#ffffff; margin:0;'>Half Day</p></div>", unsafe_allow_html=True)
            stat_col3.markdown(f"<div style='background:rgba(255, 75, 75, 0.1); border:1px solid #ff4b4b; padding:15px; border-radius:8px; text-align:center;'><h3 style='color:#ff4b4b; margin:0;'>{absent}</h3><p style='color:#ffffff; margin:0;'>Absent</p></div>", unsafe_allow_html=True)
            stat_col4.markdown(f"<div style='background:rgba(136, 136, 136, 0.1); border:1px solid #888888; padding:15px; border-radius:8px; text-align:center;'><h3 style='color:#888888; margin:0;'>{off}</h3><p style='color:#ffffff; margin:0;'>Off (Sat/Sun)</p></div>", unsafe_allow_html=True)
            ...
            # 🟢 NAYA LOGIC: Specific date select karne ka option taaki click karke Checkin/out detail dekh sakein
            st.markdown("<h4 style='color: #ffffff; margin-top: 20px;'>📋 View Specific Date Attendance</h4>", unsafe_allow_html=True)
            selected_att_date = st.date_input("Select Date to view Check-In / Check-Out details", value=ist_now.date(), key="att_record_date")
            selected_att_str = selected_att_date.strftime('%d-%b-%Y')
            
            df_att = get_today_attendance_db(selected_att_str, intern_name=sel_intern, role=user_role)
            if len(df_att) == 0: 
                st.markdown(f"<p style='color: #a1a1aa; font-size: 16px; padding: 15px; background: rgba(255,255,255,0.05); border-radius: 8px; text-align: center;'>No attendance records available for {selected_att_str}.</p>", unsafe_allow_html=True)
            else: 
            ...
            # Agar subha 9 baje se pehle task submit ho raha hai, toh date aur day KAL (Yesterday) ke lagenge.
            if current_hour < 9:
                log_date_obj = now - datetime.timedelta(days=1)
            else:
                log_date_obj = now
                
            log_date = log_date_obj.strftime('%d-%b-%Y')
            log_day = log_date_obj.strftime('%A')
            ...
            with st.container(border=True):
                ...
                with col_date: st.text_input("Date", value=log_date, disabled=True, key="task_date_input")
                with col_day: st.text_input("Day", value=log_day, disabled=True, key="task_day_input")
                
                # 🟢 NAYA TWO-STEP LOGIC START
                current_task_data = get_daily_task_db(log_date, sel_intern_task)
                
                if not current_task_data:
                    # Phase 1: Morning Check-In (Plan Task)
                    ...
                    task_input = st.text_area("What are your planned tasks for today? *", height=100, key="task_desc_input")
                    
                    if st.button(" Submit Planned Task", use_container_width=True, type="primary", key="btn_submit_task_plan"):
                        if not task_input.strip(): 
                            st.error("Please enter your planned tasks.")
                        else:
                            save_planned_task_db(log_date, log_day, sel_intern_task, task_input)
                            st.success(f"Planned task submitted successfully for {sel_intern_task}!"); time.sleep(1); st.rerun()

                elif current_task_data.get("status") == "Pending":
                    # Phase 2: Evening Check-Out (Submit Result)
                    ...
                    if not is_task_time:
                        st.warning("⚠️ Evening Task Update is only active from 6:00 PM to 9:00 AM.")
                        
                    st.markdown("<span style='font-size:14px; color:#ff4b4b;'>* Mandatory Fields</span>", unsafe_allow_html=True)
                    result_input = st.text_area("Result *", height=80, key="task_result_input", help="What was the result of today's task?")
                    outcome_input = st.text_area("Outcome *", height=80, key="task_out_input")
                    extra_input = st.text_area("Extra Curriculum (Optional)", height=60, key="task_extra_input")
                    
                    st.markdown("**Did you complete the task?**")
                    col_y, col_n = st.columns(2)
                    with col_y:
                        btn_yes = st.button("✅ Yes (Completed)", use_container_width=True, type="primary", disabled=not is_task_time, key="btn_yes_task")
                    with col_n:
                        btn_no = st.button("❌ No (Incomplete)", use_container_width=True, disabled=not is_task_time, key="btn_no_task")

                    if btn_yes or btn_no:
                        if not result_input.strip() or not outcome_input.strip(): 
                            st.error("Please fill Result and Outcome fields.")
                        else:
                            final_status = "Completed" if btn_yes else "Incomplete"
                            update_actual_task_db(log_date, sel_intern_task, result_input, outcome_input, extra_input if extra_input.strip() else "-", final_status, submit_time_str)
                            st.success(f"Task marked as {final_status}!"); time.sleep(1); st.rerun()
                else:
                    # Task Already Completed
                    st.success(f"✅ You have already submitted and marked today's task as **{current_task_data.get('status')}**.")
    ...
    # 🟢 📧 Send Mail Implementation
        ...
        with st.container(border=True):
            st.markdown("<p style='color: #a1a1aa;'>Attach your task logs (CSV) and send them directly to the HR department.</p>", unsafe_allow_html=True)
            
            # Email fields - 'From' picks intern's email, 'To' is forced to HR
            c1, c2 = st.columns(2)
            with c1:
                st.text_input("From (Your Email)", value=user_email, disabled=True, key="mail_from")
            with c2:
                st.text_input("To (HR Email)", value="hr@pararobo.in", disabled=True, key="mail_to")
                
            mail_subject = st.text_input("Subject *", placeholder="e.g., Monthly Task Logs", key="mail_sub")
            mail_body = st.text_area("Message / Remarks", placeholder="Type your message here...", height=120, key="mail_body")
            
            # CSV File Upload Restrict
            mail_attachment = st.file_uploader("Attach CSV File *", type=['csv'], key="mail_attach")
            
            st.markdown("<br>", unsafe_allow_html=True)
            
            if st.button("📤 Send Mail", type="primary", use_container_width=True):
                if not mail_attachment:
                    st.error("⚠️ Please attach a CSV file before sending!")
                else:
                    with st.spinner("Preparing and sending professional email to HR..."):
                        try:
                            # Extract Intern Name for Professional Template
                            intern_name = st.session_state.get('current_user_name', 'Intern')
                            current_month = datetime.datetime.now().strftime("%B %Y")
                            
                            # Create the Email Message Structure
                            msg = EmailMessage()
                            
                            # Auto-generated clean Subject
                            clean_subject = mail_subject if mail_subject.strip() else f"Monthly Task Logs - {intern_name}"
                            msg['Subject'] = clean_subject
                            
                            # Sender (System) and Receiver (HR)
                            # 🟢 FIX: HR ko Intern ka naam dikhega, bhale hi email system ka ho
                            msg['From'] = f"{intern_name} (via Alpha CRM) <{SMTP_EMAIL}>"
                            msg['To'] = "hr@pararobo.in"
                            
                            # IMPORTANT: HR jab reply karega toh seedha Intern ke email par jayega
                            msg.add_header('reply-to', user_email) 

                            # Auto-Generated Professional Body Template
                            formatted_body = f"Dear HR Team,\n\nPlease find attached the task logs for {intern_name} for the period of {current_month}.\n\nRemarks/Message from Intern:\n{mail_body if mail_body.strip() else 'No additional remarks provided.'}\n\nBest Regards,\n{intern_name}\n(Sent via Alpha CRM)"
                            msg.set_content(formatted_body)
                            
                            # CSV Attachment Handling (Rename file to standard format)
                            safe_name = intern_name.replace(" ", "_")
                            standard_filename = f"TaskLogs_{safe_name}_{current_month.replace(' ', '')}.csv"
                            
                            msg.add_attachment(
                                mail_attachment.getvalue(), 
                                maintype='text', 
                                subtype='csv', 
                                filename=standard_filename
                            )
                            
                            # Send the Mail using Gmail SMTP (Reverted to 465 SSL)
                            with smtplib.SMTP_SSL('smtp.gmail.com', 465) as smtp:
                                smtp.login(SMTP_EMAIL, SMTP_PASSWORD)
                                smtp.send_message(msg)
                                
                            st.success(f"✅ Mail sent successfully to hr@pararobo.in with attachment '{standard_filename}'!")
                            
                        except Exception as e:
                            st.error(f"⚠️ Failed to send email. Error: {e}")