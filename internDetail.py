    ...
    if main_tab == "🧑‍🎓 Interns Information":
                    ...
                    with c3:
                        ...
                        with bc1:
                            if st.button("✅ Approve", key=f"app_{row['rowid']}", use_container_width=True):
                                if conf_input.strip().lower() == row['intern_name'].strip().lower():
        ...
        with st.container(border=True):
            ...
            with m_col1:
                ...
                st.markdown(f"<span style='font-size:12px; color:#a1a1aa;'>Type <b>{man_intern}</b> to confirm action:</span>", unsafe_allow_html=True)
            ...
            with b_col1:
                if st.button("✅ Mark as Present", type="primary", use_container_width=True, key="btn_man_present"):
                    ...
                    elif man_conf_input.strip().lower() != man_intern.strip().lower():
