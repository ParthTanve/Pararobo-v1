...
def show_intern_page():
    ...
    if main_tab == "🧑‍🎓 Interns Information":
        ...
        if log_type == "📅 Attendance Log":
            ...
            # 🟢 NAYA LOGIC: Agar subha 9 baje se pehle task submit ho raha hai, toh date aur day KAL (Yesterday) ke lagenge.
            if current_hour < 9:
            ...
            with st.container(border=True):
                ...
                has_submitted_today = check_daily_task_exists_db(log_date, sel_intern_task)
                ...
                if has_submitted_today:
                    ...
                    if st.button(" Submit Task Log", use_container_width=True, type="primary", key="btn_submit_task", disabled=not is_task_time):
                        if not task_input.strip() or not result_input.strip() or not outcome_input.strip(): 
                            ...
                            save_task_log_db(log_date, log_day, sel_intern_task, task_input, result_input, outcome_input, extra_input if extra_input.strip() else "-", submit_time_str)
