...
def get_month_stats(intern_name):
    ...
    for doc in docs:
        ...
        try:
            ...
            if doc_date.month == current_month and doc_date.year == current_year:
...
def mark_checkin_db(date_str, intern_name, time_str, photo_bytes, is_late=False):
...
def show_intern_page():
    ...
    if main_tab == "🧑‍🎓 Interns Information":
        ...
        if log_type == "📅 Attendance Log":
            with st.container(border=True):
                ...
                if photo:
                    ...
                    if st.button(f" Confirm {att_action}", use_container_width=True, disabled=(is_disabled or is_att_disabled), key="btn_confirm_att"):
                        ...
                        if att_action == "Check-In":
                            applied_status = mark_checkin_db(date_str, sel_intern, time_str, photo_bytes, is_late=is_late_checkin)
                            if applied_status:
                                if is_late_checkin:
            ...
            present, absent, off, half_day, late_count = get_month_stats(sel_intern)
