...
def init_client_db():
    ...
    try:
        cursor.execute("ALTER TABLE clients ADD COLUMN total_revenue REAL")
    except sqlite3.OperationalError:
        pass
...
def save_new_client(c_id, c_name, email, p_name, status, c_info, contacts, gst, bill_addr, agree, s_date_cli, e_date_cli, t_lead, t_members, file_data, file_name, photo, p_category, p_rev, s_date_proj, e_date_proj):
    ...
    # 1. Save all details to clients table
    try:
        cursor.execute('''
            INSERT INTO clients (client_id, client_name, email, project_name, status, company_info, contacts, gst_details, billing_address, agreements, start_date, end_date, team_lead, team_members, doc_data, doc_name, extension_reason, photo_data, total_revenue)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (c_id, c_name, email, p_name, status, c_info, contacts, gst, bill_addr, agree, s_date_cli, e_date_cli, t_lead, t_members, file_data, file_name, "-", photo, p_rev))
        conn.commit()
    except sqlite3.IntegrityError:
        conn.close()
        return "duplicate_id"
    
    # 2. Automatically copy project data into projects table (Cross-DB Sync)
    try:
        ...
        cursor_proj.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (p_id, p_name, p_category, c_info, c_name, p_rev, 0.0, float(p_rev), tools_used, t_members, s_date_proj, e_date_proj, progress, status, file_data, file_name, t_lead))
        conn.commit()
