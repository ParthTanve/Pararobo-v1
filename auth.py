...
# ==========================================
# 🟢 NAYA FIX: Data ko cache kiya taaki Firebase bar-bar call na ho
...
def set_intern_password(email, password):
    ...
    check_user_email.clear() # 🟢 NAYA FIX: Password update hone par cache clear karna
                        ...
                        if not user_data["password"]:
                            ...
                            if st.button("Set Password & Login", use_container_width=True, type="primary"):
                                if p1 and p1 == p2:
                                    ...
                                    # ✅ URL ki jagah direct hidden memory mein session_token save kar diya
                                    st.session_state.update({'logged_in': True, 'current_user_name': user_data['name'], 'user_email': email_input, 'user_role': role, 'session_token': token})
                            ...
                            if st.button("Login", use_container_width=True, type="primary"):
                                if hash_password(pw) == user_data["password"]:
                                    token = uuid.uuid4().hex
                                    create_session(token, user_data['name'], email_input, role)
                                    # ✅ URL se hide karke memory mein daal diya
                                    st.session_state.update({'logged_in': True, 'current_user_name': user_data['name'], 'user_email': email_input, 'user_role': role, 'session_token': token})
