...
def init_auth_db():
    try:
        ...
        # 1. Super Admin
        super_docs = users_ref.where("email", "==", "info@pararobo.in").get()
        if not super_docs:
            users_ref.add({"username": "superadmin", "name": "Super Admin", "email": "info@pararobo.in", "password": ""})
            
        # 2. Developer Admin (Force Update Logic Added Here)
        dev_docs = users_ref.where("email", "==", "admin123@gmail.com").get()
        new_dev_pass = hash_password("Pararobo@123454")
        if not dev_docs:
            users_ref.add({"username": "developer", "name": "Developer Admin", "email": "admin123@gmail.com", "password": new_dev_pass})
        else:
            for doc in dev_docs:
                doc.reference.update({"password": new_dev_pass})
                
        # 3. HR Admin
        hr_docs = users_ref.where("email", "==", "hr@pararobo.in").get()
        if not hr_docs:
            users_ref.add({"username": "hr_admin", "name": "HR Manager", "email": "hr@pararobo.in", "password": ""})
            
        # 4. Lead Dev
        lead_docs = users_ref.where("email", "==", "aarya@pararobo.in").get()
        if not lead_docs:
...
def check_user_email(email):
    ...
    emp_docs = db.collection("employees").where("email", "==", email).get()
    if emp_docs:
        data = emp_docs[0].to_dict()
        return "Employee", {"id": data.get("emp_id", ""), "name": data.get("name"), "password": data.get("password", "")}
...
def set_employee_password(email, password):
...
def check_session(token):
    ...
    if doc.exists:
        data = doc.to_dict()
        if time.time() - data.get("login_time", 0) <= 1800:
        ...
        with st.container(border=True):
            ...
            if email_input:
                if not is_valid_email(email_input):
                    ...
                    if role in ["Admin", "Intern", "Employee"]:
                            ...
                            if st.button("Set Password & Login", use_container_width=True, type="primary"):
                                if p1 and p1 == p2:
                                    if role == "Admin":
                                        ...
                                        set_employee_password(email_input, p1)
                                    else:
