...
# 🟢 NAYA LOGIC: Edit karne ke liye naya function
def update_lead_details_db(lead_id, name, contact, email, platform, purpose, l_type):
    db.collection("leads").document(lead_id).update({
        "lead_name": name,
        "contact": contact,
        "email": email,
...
# 🟢 NAYA LOGIC: Edit Lead Dialog Modal
@st.dialog("✏️ Edit Lead Details", width="large")
def edit_lead_dialog(lead_data):
    st.markdown("<p style='color: #a1a1aa;'>Update the lead details below.</p>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        new_name = st.text_input("Lead Name *", value=lead_data.get('lead_name', ''), key=f"edit_name_{lead_data['lead_id']}")
        new_contact = st.text_input("Contact Number *", value=lead_data.get('contact', ''), key=f"edit_contact_{lead_data['lead_id']}")
        p_opts = ["WhatsApp", "Facebook", "Instagram", "Email", "Cold Call", "LinkedIn", "Walkin"]
        p_idx = p_opts.index(lead_data.get('platform', "WhatsApp")) if lead_data.get('platform') in p_opts else 0
        new_platform = st.selectbox("Source Platform *", p_opts, index=p_idx, key=f"edit_plat_{lead_data['lead_id']}")
        
    with col2:
        new_email = st.text_input("Email ID *", value=lead_data.get('email', ''), key=f"edit_email_{lead_data['lead_id']}")
        t_opts = ["Hot", "Warm", "Cold", "Not Connected"]
        t_idx = t_opts.index(lead_data.get('lead_type', "Cold")) if lead_data.get('lead_type') in t_opts else 2
        new_type = st.selectbox("Lead Status *", t_opts, index=t_idx, key=f"edit_type_{lead_data['lead_id']}")
        
    new_purpose = st.text_area("Lead Purpose / Requirement *", value=lead_data.get('purpose', ''), key=f"edit_purp_{lead_data['lead_id']}", height=100)
    st.markdown("<br>", unsafe_allow_html=True)
    
    if st.button("💾 Save Changes", type="primary", use_container_width=True):
        if new_name and new_contact and new_email and new_purpose:
            if not new_name.replace(" ", "").isalpha():
                st.error("⚠️ Lead Name should only contain alphabets.")
            elif not (new_contact.isdigit() and len(new_contact) == 10):
                st.error("⚠️ Contact number must be exactly 10 digits.")
            elif not is_valid_email(new_email):
                st.error("⚠️ Invalid Email Format!")
            else:
                update_lead_details_db(lead_data['lead_id'], new_name, new_contact, new_email, new_platform, new_purpose, new_type)
...
def show_lead_page():
    ...
    if len(df) == 0:
        ...
        # 🟢 Columns width thodi adjust ki hai taaki aakhiri me Edit ka button fit ho jaye
        st.markdown("<hr style='margin: 0px; border-top: 2px solid rgba(255,255,255,0.2);'>", unsafe_allow_html=True)
        h_cols = st.columns([0.4, 1.1, 1.1, 1.4, 1.0, 1.1, 1.4, 0.9, 0.8, 0.8, 0.7], vertical_alignment="center")
        ...
        with h_cols[10]: st.markdown("**Edit**")
        st.markdown("<hr style='margin: 0px; border-top: 2px solid rgba(255,255,255,0.2);'>", unsafe_allow_html=True)
        
        leads_to_delete = []
        
        for idx, row in df.iterrows():
            cols = st.columns([0.4, 1.1, 1.1, 1.4, 1.0, 1.1, 1.4, 0.9, 0.8, 0.8, 0.7], vertical_alignment="center")
            ...
            # 🟢 NAYA LOGIC: Edit Button in the last column
            with cols[10]:
                if st.button("✏️", key=f"edit_btn_{row['lead_id']}", help="Edit this lead"):
