...
if not st.session_state.get('logged_in', False):
    ...
    if session_token:
        ...
        if username:
            ...
            st.session_state['session_token'] = session_token # 🟢 Session id memory me save ki
            st.query_params.clear() # 🟢 URL se session id turant hide kar di
        else:
...
# 🟢 Catch URL clicks (From KPI cards) and move it to hidden memory
if "page" in st.query_params:
    st.session_state["current_page"] = st.query_params.get("page")
    st.query_params.clear() # 🟢 URL turant saaf karega

# 🟢 NAYA FIX: URL ki jagah ab internal memory se page load hoga
current_page = st.session_state.get("current_page", "Dashboard")
user_role = st.session_state.get('user_role', 'Admin')

def navigate(page_name):
    # 🟢 NAYA FIX: URL query update karne ki jagah session update hoga
...
def create_clickable_kpi_card(title, value, delta, is_negative=False):
    ...
    # 🟢 NAYA FIX: KPI link URL mein sirf page pass hoga, jo loop hook se automatic hide ho jayega
    link_url = f"?page={safe_title}"
...
with st.sidebar:
    ...
    if st.button("🚪 Logout", use_container_width=True, type="primary"):
        # 🟢 NAYA FIX: URL se session hat chuka hai, toh token memory se uthana padega
