...
from config import db # 🟢 Added to verify actual intern role

# Page configuration must be the first Streamlit command
st.set_page_config(page_title="Alpha2.O", page_icon="crmgif.gif", layout="wide", initial_sidebar_state="expanded")
...
# ==========================================
# AUTO LOGIN CHECK (30 MINUTE PERSISTENCE & REFRESH SAFE)
# ==========================================
if not st.session_state.get('logged_in', False):
    ...
    if session_token:
        ...
        if username:
            ...
            # 🟢 URL clear nahi karenge taaki Refresh par log out na ho
        else:
...
# 🟢 Get Page Directly from Query Params
current_page = st.query_params.get("page", "Dashboard")
user_role = st.session_state.get('user_role', 'Admin')

# 🟢 NAYA FIX: Fetch specific intern role for navigation logic
if user_role == 'Intern' and 'intern_specific_role' not in st.session_state:
    try:
        docs = db.collection("interns").where("email", "==", st.session_state.get('user_email', '')).get()
        if docs:
            st.session_state['intern_specific_role'] = docs[0].to_dict().get("role", "")
        else:
            st.session_state['intern_specific_role'] = ""
    except Exception:
        st.session_state['intern_specific_role'] = ""

intern_specific_role = st.session_state.get('intern_specific_role', '')
...
def create_clickable_kpi_card(title, value, delta, is_negative=False):
    ...
    # Add page and preserve session token
    session_token = st.query_params.get("session", "")
    link_url = f"?session={session_token}&page={safe_title}" if session_token else f"?page={safe_title}"
...
with st.sidebar:
    ...
    if user_role == 'Admin':
        ...
        # 🟢 NEW FEATURE: FreeLancing Team Navigation
        with st.expander("🌍 FreeLancing Team"):
             if st.button("View Freelancers", use_container_width=True):
                navigate("FreeLancing Team"); st.rerun()
    ...
    # Intern sees strictly their required modules
        ...
        # 🟢 FIX: Digital Marketing aur Sales & Marketing ko "Leads" dikhega
        if intern_specific_role in ["Digital Marketing", "Sales & Marketing"]:
            with st.expander("🎯 Leads"):
                if st.button("View Leads", use_container_width=True):
                    navigate("Lead Detail"); st.rerun()
...
if current_page == "Dashboard":
    ...
    with logo_col:
        try: st.image(logo_path, width=500)
        except: pass
    with text_col:
        try: st.image(text_path, width=500)
        except: pass
...
# 🟢 NEW FEATURE: FreeLancing Team Route
elif current_page == "FreeLancing Team":
    try:
        import freelancingTeam
        freelancingTeam.show_freelance_page()
    except ImportError:
        st.markdown("<h1 style='color: #ffffff;'>🌍 FreeLancing Team</h1>", unsafe_allow_html=True)
        st.markdown("---")
        st.info("The FreeLancing Team module is under construction. Create a file named `freelancingTeam.py` to link this section.")
